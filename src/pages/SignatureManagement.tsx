...
const SignatureManagement = () => {
  ...
  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from("pathologist_signatures").select("*").order("created_at");
    setSignatures(data || []);
    setLoading(false);
  };
  ...
  const handleEdit = (s: any) => {
    setEditId(s.id);
    setForm({ pathologist_name: s.pathologist_name, designation: s.designation || "", qualification: s.qualification || "" });
    setSigFile(null);
    setDialogOpen(true);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setEditId(null); setForm({ pathologist_name: "", designation: "", qualification: "" }); setSigFile(null); setDialogOpen(true); }}>
          ...
          {loading ? <div className="flex justify-center p-8"><Loader2 className="h-6 w-6 animate-spin" /></div> : (
            <Table>
              ...
              <TableBody>
                ...
                {signatures.length === 0 && <TableRow><TableCell colSpan={5} className="text-center py-8 text-muted-foreground">No pathologists added yet</TableCell></TableRow>}
              </TableBody>
            </Table>
          )}
        </CardContent>
