-- Store which phlebotomist completed the home visit (shown in Registered Patients expand).
ALTER TABLE public.patient_registrations
  ADD COLUMN IF NOT EXISTS completing_phlebo_name text;

COMMENT ON COLUMN public.patient_registrations.completing_phlebo_name IS
  'Phlebotomist name recorded when registering from a home visit completion flow.';

-- Persist completing_phlebo_name through register_patient_atomic (same LIMS invoice path).
