-- Concurrent-safe invoice allocation for multi-receptionist registration.
-- 1) Counter row is locked/incremented atomically (GREATEST with max existing).
-- 2) register_patient_atomic always assigns the number in the same transaction
--    as the INSERT, so two receptionists cannot receive the same invoice.
...
BEGIN
  ...
  -- Row lock on invoice_counter serializes concurrent callers in this transaction.
  INSERT INTO public.invoice_counter (date_key, last_sequence)
  VALUES (today, max_existing + 1)
  ON CONFLICT (date_key) DO UPDATE
  SET last_sequence = GREATEST(public.invoice_counter.last_sequence, max_existing) + 1
...
BEGIN
  ...
  -- Always allocate inside this transaction (ignore client-supplied numbers).
