...
AS $$
  ...
  ORDER BY
    ...
    CASE
      WHEN UPPER(TRIM(c.location)) = 'PH VESU' THEN
        CASE
          WHEN c.visit_date ~ '^\d{1,2}-\d{1,2}-\d{4}$' THEN
    ...
    CASE
      WHEN UPPER(TRIM(c.location)) = 'PH VESU' THEN
        REGEXP_REPLACE(c.bill_number, '\D', '', 'g')
      ELSE '0'
    END DESC,
    c.created_at DESC
  LIMIT p_page_size
...
CREATE OR REPLACE FUNCTION public.get_crm_contacts_count(
  p_location text DEFAULT 'ALL',
  p_tag text DEFAULT 'ALL',
  p_search text DEFAULT ''
)
