-- Pending Dispatch: regs with >=1 approved (not yet dispatched) report.
-- Date range by default; optional include older (created_at < p_date_from).
...
-- Drop older 2-arg overload if present
DROP FUNCTION IF EXISTS public.lims_dispatch_pending_dispatch_ids(text, timestamptz);