-- Fix pipeline hover "Unknown" rows: package/combo/profile container IDs are not leaf tests.
-- When sample tubes exist, use tube-expanded leafs only (same rule as expandRegistrationTests).
-- Before collection, show billed selection names from registration JSON.
...
BEGIN
  ...
  reg_names AS (
    SELECT DISTINCT ON (id)
      NULLIF(elem->>'test_id', '')::uuid AS id,
      NULLIF(trim(elem->>'test_name'), '') AS test_name
    FROM jsonb_array_elements(v_tests) elem
    WHERE NULLIF(elem->>'test_id', '') IS NOT NULL
    ORDER BY id, (elem->>'test_name') NULLS LAST
  ),
  -- Prefer tube-expanded leafs once collection created tubes.
  -- Do NOT union billed package/combo/profile container IDs (those are not in public.tests).
  leafs AS (
  ...
  base AS (
    SELECT
      lu.id AS test_id,
      COALESCE(NULLIF(t.test_name, ''), rn.test_name, 'Unknown') AS test_name,
    ...
    LEFT JOIN reg_names rn ON rn.id = lu.id
...
COMMENT ON FUNCTION public.lims_patient_test_pipeline_status(uuid) IS
  'Latest per-test pipeline status for one registration (hover). Tube leafs only after collection; billed names before.';
