-- Pending Dispatch: also show partially_dispatched bills (e.g. some tests
-- dispatched, another accepted tube still missing results). Previously only
-- rows with status=approved qualified, so invoices like 2608120001 vanished
-- from Pending even though the visit was incomplete.
--
-- When the user searches (invoice/name/mobile), skip the date window so a
-- known bill is findable without flipping Show older.
...
AS $$
  ...
  WHERE COALESCE(pr.bill_cancelled, false) = false
    ...
    AND (
      ...
      OR COALESCE(pr.status, '') = 'partially_dispatched'
    )
    AND (
